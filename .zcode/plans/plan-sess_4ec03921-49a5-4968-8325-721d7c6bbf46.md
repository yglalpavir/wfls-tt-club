执行动作：用 Write 工具以以下内容覆盖 plan.md（旧版 08-24 审计已在 git 历史中，无需备份；不提交 git，除非你另行要求）。
...
13. 搜索摘要"排名：/胜率："硬编码（common.js:487）；搜索占位"搜索..."与"加载中..."默认值（common.js:44,58）；图表 tooltip `${raw} 场` 与轴"年/月"（data-viz-extra.js:139,345,348,412；wtt_dataviz-extra.js:33,36,993,1224）；draws-viewer.js:93-97 工具 title。
...
20. 全局状态债（长期）：~45 个可变全局、async 交换窗口期 club/WTT 数据互见、WTT 三个人口加载函数无 single-flight、_seasonStartCache 按引用键控、loadRankingData 三重定义靠脚本顺序裁决。
...
- [ ] 删 data/package.json、data/package-lock.json
- [ ] 修 5 处标题闭标签（ranking.html:75、wtt_ranking.html:66、wtt_dataviz.html:282/284、wtt_assoc.html:352）
- [ ] news n11 补正文或 visible:false；跑 `python tools/sync_content.py` 重建索引
- [ ] README 补充"同日多局正常录入"口径说明

### Phase 1 — 引擎/前端 bug（P1-1 ~ P1-12）
- [ ] 删 personal-stats 死 LUT（P1-1）；`*0.8`→LOSER_POINT_MULTIPLIER ×14（P1-2）；重放衰减基准统一（P1-3）
- [ ] wttRenderScoreDetail 改 wttWithDataContext（P1-4）；补赛制参数 ×6（P1-5）
- [ ] getSeasonForDate 首赛季回退（P1-6）；initialScore Number 强转 + typeof 判数（P1-7）
- [ ] 日期格式校验 + 未来记录告警（P1-8）；resp.ok ×7（P1-9）；wttInitialized 失败复位（P1-10）
- [ ] 版本下拉 handler 泄漏（P1-11）；公共页统一错误态 + noscript（P1-12）

### Phase 2 — 工具链门禁
- [ ] sync_content.py：--check 产物 diff（#1）、GBK stdout（#2）、folder≠id 硬错误（#3）、孤儿快照检测（#4）、check/sync 行为一致（#5）
- [ ] ci_validate.py：结构守卫（#6）、bonus 分数校验（#7）、赛制校验三件（#8）、赛季 ISO/重叠/snapshotDates（#9）、players/draws schema（#10）
- [ ] 验证：手改一处 index.json → --check 应红；构造缺"负者"记录 → 应 FAIL 而非 traceback

### Phase 3 — 体验与卫生
- [ ] i18n：WTT label 运行期解析接 getNodeDisplayLabel（#12）；#13 全部硬编码串进词典；#14 键错用纠正；顺带把 formatSnapshotLabel 存结构化日期
- [ ] shared-partials.js：nav/footer/QR/主题脚本收敛为 JS 注入（#16，根治错配类回归）
- [ ] a11y/SEO 零碎（#15）；CDN SRI + KaTeX 按需（#17）
- [ ] 公开面收缩（#18，见决策点 2）；媒体压缩与字体裁剪（#19，需资源重制，可分批）
- [ ] 更新 AGENTS.md（escapeHtml 描述、同日多局口径、event-coefficient 迭代提醒复核）

### Phase 4 — 长期（承接上轮未完成项）
- [ ] GitHub Actions 构建管线（预计算快照 / 预渲染 / sitemap 自动 lastmod / cache-bust）
...
| 2 | 公开面收缩范围 | 默认 git rm --cached：tools/ 爬虫与原始数据、docs/ 内部审计、data/_legacy/、qa 版本快照改为不部署（保留本地）；tools/ 两个 Python 工具保留跟踪 |
| 3 | Phase 4 取舍 | 先构建管线，其余按需 |

## 五、验证方式

1. 每阶段收尾：`python tools/sync_content.py --check` + `python tools/ci_validate.py` 全绿；Phase 2 后用负面用例验证门禁真能红
2. 数值一致性：修 P1-2/P1-3 后，同一球员在排名弹窗、个人页、数据可视化三处每局得失分一致（数据_viz 对比表 vs 弹窗抽 3 局核对）
...
收尾：Write 完成后向用户总结本轮新发现的关键点（回归 5 处、`*0.8` 漂移 14 处、npm 残留、n11 空正文、门禁缺口）及 plan.md 已更新。