...
- **积分明细模态框**：点击姓名查看详细记录，按快照日期截断；记录携带「比分/局分」时自动出现比分列（悬停查看逐局）
- **双重积分显示**：外侧显示原始变动，括号内显示定格后变动
- **积分调整记录**：支持"比赛结果加分"类型
- **表头点击排序**：支持按序号、姓名、积分、积分变化、排名变化、场次、胜率排序
- **单打/双打切换**：双打组合积分榜与单打榜同页切换（`?mode=doubles` 直达），两套时间线一次算好、切换零请求；组合行逐成员链个人页，收据图标打开组合积分明细
...
- **赛季选择器**：所有可见赛季可切换，默认当前赛季（进行中标记，统计截至今天）；支持 `?season=<赛季id>` 深链直接分享某个赛季
- **积分变化榜**：赛季初始 vs 最新快照的积分/名次变化（仅当季有对局者），配 TOP 10 净变化条形图
- **连胜榜 / 出勤榜**：最长连胜与当前连胜、当季场次与胜率
- **单场得分之最**：当季原始得分（未衰减）最高的一场
- **局分亮点**：含「局分」记录的打满决胜局 / 逆转翻盘 / 最大单局分差统计
- **积分调整审计**：当季「比赛结果加分」全部记录、调整前积分与每人净调整汇总，公开可查
- 数据口径与排行榜引擎完全一致；入口在数据可视化页底部

### WTT 赛季总结（wtt_season_review.html）

- 彩蛋玩法：五大项目（ms/ws/md/wd/xd）按年度赛季出总结，`?cat=` 切换项目，`?season=` 直达某年
- 口径与 WTT 排名一致（零和积分、无时间衰减、月度快照）；积分调整审计为俱乐部专属，WTT 模式不渲染
- 入口在 WTT Hub（`wtt_hub.html`）类目卡片，与协会数据并列
...
- WTT 比赛记录同样支持可选字段 `比分`（如 "3-1"）与 `局分`（如 ["11-9","8-11"]，胜者视角）：比赛详情页大比分/逐局小比分、排名积分明细与球员页比分列（有数据时自动出现）随之展示；`ci_validate.py` 对 `wtt_data/*/score-log-*.json` 与俱乐部 log 同口径校验（WTT 记录无赛制字段，跳过局数匹配检查）
...
| `赛制系数` | object | **保留键**。赛制倍率配置（仅社团积分生效）。数值条目 = 全局统一倍率，如 `"bo3": 0.8`；对象条目 = 该赛事类型专属倍率表，如 `"普通": {"bo3": 0.7, "bo5": 1.2}`，仅覆盖列出的赛制、其余回退全局；留空表示全部按 1 |
| `默认赛制` | object | **保留键**。各赛事类型 `default` 赛制所映射的赛制，如 `"普通": "bo3"`；未配置映射的类型 default 按 1 |

> 保留键的值为对象而非数值，遍历该文件时请按「值为 number」过滤出真实赛事类型（前端与 CI 已做过滤）。倍率取值优先级：**类型专属 → 全局 → 1**。赛制系数机制不影响 WTT 积分。
...
```text
胜者加分 = 基础积分 × 赛事系数 × 赛制系数 × 胜者时间权重
负者扣分 = 基础积分 × 赛事系数 × 赛制系数 × 0.8 × 负者时间权重
（时间权重按双方各自的"球员×类型"批次分别定格/衰减，一般不同）
积分地板 = max(1200, 计算结果)
```
