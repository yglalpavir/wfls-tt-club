...
export class ittfPingPong {
    ...
    ;
    async top100WTTFrontdoor(type, gender, category) {
        try {
            ...
            let response = await fetch(WTTFrontdoorBaseUrl, {
                method: 'GET',
                headers: {
    ...
    ;
    async remainingRanksWTTApi(type, gender, category, topN, requestDelay) {
        ...
        try {
            while (shouldContinue) {
                ...
                // Perform the fetch
                const response = await scraper.get(batchUrl.href, {
                    headers: {
                        ...this.keys,
                        'Origin': 'https://www.worldtabletennis.com',
                        'Referer': 'https://www.worldtabletennis.com/',
   ...
   *
   */
    async currentRankings(type, gender, category, topN = 100, requestDelay = 2000) {
        try {
            ...
            let topRanks = [];
            let top100AllTabsJson = await this.top100WTTFrontdoor(normalizedType, normalizedGender, normalizedCategory);
            ...
            if (typeof normalizedTopN === 'number' && normalizedTopN <= 100) {
                ...
                const remainderJson = await this.remainingRanksWTTApi(normalizedType, normalizedGender, normalizedCategory, normalizedTopN, requestDelay);
                if (remainderJson)
   ...
   */
    async playerIttfId(searchName) {
        try {
            ...
            let response = await fetch(this.allPlayersUrl);
            let responseJson = await response.json();
            ...
            if ('playerFullName' in searchName) {
                ...
                try {
                    let searchResult = output.filter((entries) => entries.PlayerGivenName === searchName.playerGivenName?.charAt(0).toUpperCase() + searchName.playerGivenName.slice(1).toLowerCase());
                    let filteredSearchResult = searchResult.map(({ IttfId, PlayerFamilyNameFirst }) => ({ IttfId, PlayerFamilyNameFirst }));
                    if (filteredSearchResult.length === 0) {
                        throw Error;
                    }
                ...
                try {
                    let searchResult = output.filter((entries) => entries.PlayerFamilyName === searchName.playerFamilyName?.toUpperCase());
                    let filteredSearchResult = searchResult.map(({ IttfId, PlayerFamilyNameFirst }) => ({ IttfId, PlayerFamilyNameFirst }));
                    if (filteredSearchResult.length === 0) {
                        throw Error;
                    }
   ...
   *
   */
    async playerProfile(searchMethod, options = { includeExtendedDetails: false }) {
        try {
            ...
            if ('playerFullName' in searchMethod) {
                ...
                const playerArray = await this.playerIttfId({ playerFullName: searchMethod.playerFullName.replace(/^(\S+)\s+(.+)$/, (_, lastname, firstname) => `${lastname.toUpperCase()} ${firstname[0].toUpperCase()}${firstname.slice(1).toLowerCase()}`) });
            ...
            if (options.includeExtendedDetails === true) {
                let extraResponse = await fetch(this.allPlayersUrl);
                let extraJson = await extraResponse.json();
