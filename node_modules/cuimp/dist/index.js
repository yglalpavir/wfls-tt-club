...
var BROWSER_LIST = ["chrome", "firefox", "edge", "safari"];
var ARCHITECTURE_LIST = ["x64", "arm64", "arm"];
var PLATFORM_LIST = ["linux", "windows", "macos"];

// src/validations/descriptorValidation.ts
var validateDescriptor = (descriptor) => {
...
function isMuslLibc() {
  ...
  try {
    ...
    try {
      const lddOutput = execSync("ldd --version 2>&1", { encoding: "utf8", timeout: 5e3 });
...
function getPackageDir() {
  ...
  try {
    const packageJsonPath = __require.resolve("../../package.json");
...
var extractVersionNumber = (filename) => {
  const match = filename.match(/(\d+)/);
  return match ? parseInt(match[1], 10) : 0;
};
var validateParameters = (browser, architecture, platform) => {
...
var findBinaryWithVersion = (browser, version) => {
...
var findExistingBinary = (browser = "") => {
  const isWindows = process.platform === "win32";
...
var downloadAndExtractBinary = async (browser, architecture, platform, version, logger) => {
  try {
    ...
    const searchDirs = platform === "windows" ? [path.resolve(binariesDir, "bin"), binariesDir] : [binariesDir];
...
var parseDescriptor = async (descriptor, logger = console, autoDownload = true) => {
  try {
...
var Cuimp = class {
  ...
  isBinaryExecutable(binaryPath) {
    try {
      ...
      const stats = fs2.statSync(binaryPath);
      if (!stats.isFile()) {
        return false;
      }
      ...
      if (process.platform !== "win32") {
        ...
        const isExecutable = (mode & fs2.constants.S_IXUSR) !== 0 || (mode & fs2.constants.S_IXGRP) !== 0 || (mode & fs2.constants.S_IXOTH) !== 0;
        return isExecutable;
      }
      return true;
    } catch (error) {
...
var cuimp_default = {
  get: async (url, config) => {
    const { get: get2 } = await import("./index.js");
    return get2(url, config);
  },
  post: async (url, data, config) => {
    const { post: post2 } = await import("./index.js");
    return post2(url, data, config);
  },
  put: async (url, data, config) => {
    const { put: put2 } = await import("./index.js");
    return put2(url, data, config);
  },
  patch: async (url, data, config) => {
    const { patch: patch2 } = await import("./index.js");
    return patch2(url, data, config);
  },
  delete: async (url, config) => {
    const { del: del2 } = await import("./index.js");
    return del2(url, config);
  },
  head: async (url, config) => {
    const { head: head2 } = await import("./index.js");
    return head2(url, config);
  },
  options: async (url, config) => {
    const { options: options2 } = await import("./index.js");
    return options2(url, config);
  },
  request: async (config) => {
    const { request: request2 } = await import("./index.js");
...
function parseBatFile(batFilePath) {
  const content = fs3.readFileSync(batFilePath, "utf8");
...
function runBinary(binPath, args, opts) {
  return new Promise((resolve, reject) => {
    ...
    if (isWindows && isBatFile) {
      try {
        const batDir = path2.win32.dirname(cleanPath);
        const searchedPaths = [];
        let curlExePath = null;
        let candidatePath = path2.win32.join(batDir, "curl.exe");
        searchedPaths.push(candidatePath);
        if (fs3.existsSync(candidatePath)) {
          curlExePath = candidatePath;
        }
        if (!curlExePath) {
          const parentDir = path2.win32.dirname(batDir);
          candidatePath = path2.win32.join(parentDir, "curl.exe");
          searchedPaths.push(candidatePath);
          if (fs3.existsSync(candidatePath)) {
            curlExePath = candidatePath;
          }
        ...
        if (!curlExePath) {
          try {
            const files = fs3.readdirSync(batDir);
            const curlExe = files.find((f) => f.toLowerCase() === "curl.exe");
            if (curlExe) {
              curlExePath = path2.win32.join(batDir, curlExe);
            }
          } catch {
          }
        }
        if (curlExePath && fs3.existsSync(curlExePath)) {
    ...
    if (isWindows && !finalArgs.includes("--cacert") && !finalArgs.includes("-k")) {
      const binDir = path2.dirname(actualBinPath);
      const caBundlePath = path2.join(binDir, "curl-ca-bundle.crt");
      if (fs3.existsSync(caBundlePath)) {
    ...
    if (needsShell) {
      ...
      if (!isWindowsAbsolutePath && !path2.isAbsolute(normalizedPath) && hasPathSeparator) {
        normalizedPath = path2.resolve(normalizedPath);
      } else if (isWindowsAbsolutePath || path2.isAbsolute(normalizedPath)) {
        if (isWindowsAbsolutePath) {
          normalizedPath = path2.win32.normalize(normalizedPath);
        } else {
...
var CurlExitCode = /* @__PURE__ */ ((CurlExitCode2) => {
...
var CookieJar = class {
  ...
  constructor(option) {
    if (typeof option === "string") {
      ...
      const tempDir = path3.join(os2.tmpdir(), "cuimp-cookies");
      if (!fs4.existsSync(tempDir)) {
        fs4.mkdirSync(tempDir, { recursive: true });
      }
      const uniqueId = `${Date.now()}-${Math.random().toString(36).substring(2, 9)}`;
      this.cookieFilePath = path3.join(tempDir, `cookies-${uniqueId}.txt`);
  ...
  ensureFileExists() {
    ...
    const dir = path3.dirname(this.cookieFilePath);
    if (!fs4.existsSync(dir)) {
      fs4.mkdirSync(dir, { recursive: true });
    }
    if (!fs4.existsSync(this.cookieFilePath)) {
      fs4.writeFileSync(
        this.cookieFilePath,
  ...
  getCookiesRaw() {
    ...
    return fs4.readFileSync(this.cookieFilePath, "utf8");
  }
  ...
  setCookie(options2) {
    ...
    fs4.appendFileSync(this.cookieFilePath, line + "\n");
  }
  /**
  ...
  deleteCookie(name, domain) {
    ...
    fs4.writeFileSync(this.cookieFilePath, filteredLines.join("\n"));
  }
  /**
   * Clean up the cookie file (for temporary files)
   */
  destroy() {
    if (this.isTemporary && fs4.existsSync(this.cookieFilePath)) {
      try {
        fs4.unlinkSync(this.cookieFilePath);
      } catch {
      }
    }
  }
