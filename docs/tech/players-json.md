...
### 3.3 `pinyin` — 拼音人名（英文界面显示）

| 项 | 值 |
| --- | --- |
| 类型 | `string` |
| 格式 | 姓（全大写）+ 名（首字母大写）、空格分隔的护照式拼法（如 `CHEN Yuxuan`）；同姓名区分档（`张宸瑞1`/`张宸瑞2`）共用同一拼音 `ZHANG Chenrui`。 |
| 用途 | 语言切换为 English（`wfls-lang.v1` = `en`）时，所有展示层的球员姓名替换为该字段：`playerDisplayName()`（`js/common.js`）统一出口，供 `linkPlayerName()`、排名表、个人主页、成员卡片、积分明细弹窗、图表标签/图例、签表卡面等使用。搜索（全站搜索、season-review 签表/每日图 chips、draws-viewer 签表搜索）同时匹配原始姓名与拼音。 |
| 约束 | **仅展示层**——数据键（score-log 姓名、积分快照 `'姓名'`、URL 参数、`data-*` 属性）一律保持 `name` 原始中文姓名；字段缺失或球员无档案时回退显示原名。`ci_validate.py` 不强制要求该字段。 |
