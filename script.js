...
async function loadRankingData() { const tb = document.getElementById('rankingFullBody'); if (!tb) return; try { const r = await fetch('ranking.json'); if (!r.ok) throw new Error('fail'); rankingTimeline = await r.json(); currentTimeIndex = 0; currentSortKey = '当前积分'; currentSortDir = 'desc'; renderTimeNodeList(); updateRankingDisplay(); setupSortListeners(); } catch(e) { console.error('排名加载失败', e); tb.innerHTML = '<tr><td colspan="7" style="text-align:center;padding:40px;color:var(--accent-red);">无法加载排名数据</td></tr>'; } }
async function loadRankingDataForViz() { try { const r = await fetch('ranking.json'); if (!r.ok) throw new Error('fail'); rankingTimeline = await r.json(); console.log('DataViz: ranking.json OK,', rankingTimeline.length, '个时间节点'); return true; } catch(e) { console.error('DataViz: ranking.json 失败', e); rankingTimeline = []; return false; } }
