#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""导入 2003 年 ITTF 赛事数据（MS/WS/MD/WD/XD）。

覆盖 13 站：8 站 Pro Tour 公开赛（ittf公开赛）、Pro Tour 总决赛（总决赛）、
男子世界杯 / 女子世界杯（世界杯）、世界锦标赛（世乒赛，含混合双打 XD）。
...
- 部分行对手/胜者为空（WTTC 资格赛、German/Danish WD、Qatar MD），跳过
- WTTC WD 一行 HERCZIG Judit 同时出现在双方，跳过（重复球员）
- China Open WS 仅 R16 起 10 行（源数据截断），按现有数据导入

用法：python tools/import_2003.py
"""
import json
import os
import re
...
RAW_DIR = os.path.join(os.path.dirname(BASE_DIR), "docs", "result_ittf_link", "2003")

EVENTS = {
    "Brazilian Open Rio de Janeiro 2003":    ("ittf公开赛", "2003-06-26", "pro4"),
    "China Open Guangzhou 2003":             ("ittf公开赛", "2003-09-11", "pro4"),
    "Croatia Open Croatia 2003":             ("ittf公开赛", "2003-01-23", "pro4"),
    "Danish Open Aarhus 2003":               ("ittf公开赛", "2003-11-12", "pro5"),
    "German Open Bremen 2003":               ("ittf公开赛", "2003-11-06", "pro4"),
    "Japan Open Kobe 2003":                  ("ittf公开赛", "2003-09-18", "pro4"),
    "Malaysian Open Johor Bahru 2003":       ("ittf公开赛", "2003-10-02", "pro4"),
    "Men's World Cup Jiangyin 2003":         ("世界杯",     "2003-10-09", "mwc"),
    "Pro Tour Grand Finals Guangzhou 2003": ("总决赛",     "2003-12-11", "grandfinals"),
    "Qatar Open Doha 2003":                  ("ittf公开赛", "2003-03-03", "pro5"),
    "Swedish Open Malmo 2003":               ("ittf公开赛", "2003-11-19", "pro5"),
    "Women's World Cup Hong Kong 2003":      ("世界杯",     "2003-12-17", "wwc"),
    "World Table Tennis Championships Paris 2003": ("世乒赛", "2003-05-19", "wttc"),
}
...
TEMPLATES = {
    ...
    "mwc": {"Qualification": 0, "QuarterFinal": 1, "SemiFinal": 2,
            "Position Draw": 2, "Final": 3},
...
OVERRIDES = {
    ...
    "FORT Carlos I": "Carlos FORT",  # 波多黎各球员，I 为名字一部分
}
        ...
        if sub in ("MS", "WS"):
            ...
            if not w2:
                ...
                if win_set == {na, nb}:
                    ...
                    continue
            # MD/WD 按字母排序（与既有 DB 一致）；XD 保留原始顺序（男在前）
            key = sub.lower()
            if sub != "XD":
...
def append_to_scorelog(cat, records):
    if cat == "ws":
        filename = "score-log-2003-ws.json"
    else:
        filename = "score-log-2003-wtt.json"
...
def main():
    ...
    for cat in ("ms", "ws", "md", "wd", "xd"):
        fn = "score-log-2003-ws.json" if cat == "ws" else "score-log-2003-wtt.json"
