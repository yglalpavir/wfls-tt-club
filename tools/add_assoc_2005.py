#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""为 2005 年 ITTF 赛事（18 站）球员补充 assoc.json 协会籍记录（MS/WS）。"""
import json
import os
import re
...
# 复用 import_2005 的标准名映射
_spec = importlib.util.spec_from_file_location("imp2005", os.path.join(BASE_DIR, "import_2005.py"))
...
def extract_players(cat):
    """从全部原始文件提取 原始名 -> 国家代码集合（MS/WS，兼容断行）。"""
    want = {"ms": "MS", "ws": "WS"}[cat]
    players = {}
...
if __name__ == "__main__":
    print("补充 assoc.json (2005 十八站赛事)")
    add_to_assoc("ms")
    add_to_assoc("ws")