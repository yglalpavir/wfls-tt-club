#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""为 2015 年 ITTF 赛事（16 站）球员补充 assoc.json 协会籍记录（MS/WS）。

覆盖：MS/WS + U21MS/U21WS（一并导入 ms/ws）+ 团体世界杯 Dubai MT/WT 单打。
...
# 复用 import_2015 的标准名映射
_spec = importlib.util.spec_from_file_location("imp2015", os.path.join(BASE_DIR, "import_2015.py"))
...
if __name__ == "__main__":
    print("补充 assoc.json (2015 十六站赛事)")
    add_to_assoc("ms")
    add_to_assoc("ws")