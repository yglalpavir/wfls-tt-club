#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""导入 2014 年 ITTF 赛事数据（MS/WS/MD/WD，无 XD）。

覆盖 14 站：
- 6 站 Pro Tour Super Series → ittf白金赛（1.15）：
    Kuwait、Qatar、German Magdeburg、China Open Chengdu、Korea Incheon、Japan Yokohama
- 4 站 Pro Tour Major Series → ittf常规赛（0.48）：
    Spanish Almeria、Czech Olomouc、Russian Ekaterinburg、Swedish Stockholm
- Pro Tour Grand Finals Bangkok 2014（总决赛，4 天）
- Men's World Cup Dusseldorf 2014 / Women's World Cup Linz 2014（世界杯，各 3 天）
- World Team Table Tennis Championships Tokyo 2014（世乒赛团体，8 天；MT/WT → ms/ws，
  2014 团体赛无双打行）

姓名规范化规则（遵循 wtt_data/player-name-format.md）：同 2013 导入。
U21MS/U21WS 按用户要求一并导入 ms/ws（与正式单打同模板、同事件类型）。
数据已知问题（按要求忽略数量差，畸形行跳过）：
- 部分双打行缺一名搭档/胜者（MD/WD 数据缺口），跳过
- KIM Minhee (YOB=1991) (KOR) 与既有 DB 的 KIM Minhee（2005，YOB=1985）
  为两位不同球员：保留后缀为 "KIM Minhee (1991)"
- 现有 2014 ms 已有 7 条洲杯赛记录（02-21~23），按 (日期,类型,胜者,负者) 去重追加

用法：python tools/import_2014.py
"""
import json
import os
import re
...
EVENTS = {
    "World Tour Kuwait Open Kuwait City 2014":     ("ittf白金赛", "2014-02-12", "pro5"),
    "World Tour Qatar Open Doha 2014":             ("ittf白金赛", "2014-02-18", "pro6"),
    "World Tour German Open Magdeburg 2014":       ("ittf白金赛", "2014-03-26", "pro5"),
    "World Tour Spanish Open Almeria 2014":        ("ittf常规赛", "2014-04-02", "pro5"),
    "World Team Table Tennis Championships Tokyo 2014": ("世乒赛团体", "2014-04-28", "team8"),
    "World Tour China Open Chengdu 2014":          ("ittf白金赛", "2014-06-04", "pro5"),
    "World Tour Korea Open Incheon 2014":          ("ittf白金赛", "2014-06-11", "pro5"),
    "World Tour Japan Open Yokohama 2014":         ("ittf白金赛", "2014-06-18", "pro5"),
    "World Tour Czech Open Olomouc 2014":          ("ittf常规赛", "2014-08-27", "pro5"),
    "Women's World Cup Linz 2014":                 ("世界杯",     "2014-10-17", "wc3"),
    "Men's World Cup Dusseldorf 2014":             ("世界杯",     "2014-10-24", "wc3"),
    "World Tour Russian Open Ekaterinburg 2014":   ("ittf常规赛", "2014-11-05", "pro5"),
    "World Tour Swedish Open Stockholm 2014":      ("ittf常规赛", "2014-11-12", "pro5"),
    "World Tour Grand Finals Bangkok 2014":        ("总决赛",     "2014-12-11", "grandfinals4"),
}
...
TEMPLATES = {
    ...
    "team8": {"Qualification": 0, "Main Draw": 7},
}
...
OVERRIDES = {
    ...
    "KIM Minhee (YOB=1991)": "KIM Minhee (1991)",  # 2014 球员，与 2005 的 KIM Minhee (YOB=1985) 不同
...
def append_to_scorelog(cat, records):
    if cat == "ws":
        filename = "score-log-2014-ws.json"
    else:
        filename = "score-log-2014-wtt.json"
...
def main():
    ...
    for cat in ("ms", "ws", "md", "wd", "xd"):
        fn = "score-log-2014-ws.json" if cat == "ws" else "score-log-2014-wtt.json"
