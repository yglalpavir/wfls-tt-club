...
// 数据加载失败时的统一错误占位（替代永久转圈的"加载中"）
function showRankingLoadFail(containerId) {
    const el = document.getElementById(containerId);
    if (!el) return;
    el.innerHTML = '<div class="compare-placeholder"><i class="fa-solid fa-triangle-exclamation"></i><p style="color:var(--accent-red);">' + i18n[currentLang].rank_calc_fail + '</p><button class="btn btn-sm btn-primary" style="margin-top:14px;" onclick="location.reload()">' + (i18n[currentLang].pp_refresh || '刷新') + '</button></div>';
}
...
function initPage() {
    ...
    initCommon();
    const isRanking = !!document.getElementById('rankingFullBody'), isDataViz = !!document.getElementById('pointsTrendChart'), isWttDataViz = !!document.getElementById('wttPointsTrendChart'), isPersonalStats = !!document.getElementById('personalResult'), isPlayerPage = !!document.getElementById('playerDetailContent'), isWttPersonalStats = !!document.getElementById('wttPersonalPlayerSearchContainer') && !document.getElementById('wttPointsTrendChart'), isSeasonReview = !!document.getElementById('seasonReviewBody'), isMatchDetail = !!document.getElementById('matchDetailBody');
    ...
    if (isMatchDetail && typeof initMatchDetail === 'function') { initMatchDetail(); }
    initPdfViewer();
}
